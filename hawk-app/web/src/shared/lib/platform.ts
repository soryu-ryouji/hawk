...
export const platform =
  window.hawkShell?.platform ?? (navigator.userAgent.includes('Mac') ? 'darwin' : navigator.userAgent.includes('Windows') ? 'win32' : 'linux');

/** 是否 macOS（红绿灯/拖拽区等 Darwin 分支） */
export const isMac = platform === 'darwin';

/** 系统文件管理器称呼：macOS=Finder，Windows=资源管理器，其他=文件管理器 */
export const fileManagerName = platform === 'darwin' ? 'Finder' : platform === 'win32' ? '资源管理器' : '文件管理器';
...
const noopShell: NonNullable<Window['hawkShell']> = {
  ...
  getCloseAction: async () => 'exit' as const,
