...
| `src/api/library.rs` | `library/info` GET（显示名取 config 的 name，缺省目录名）/ PATCH（改显示名，toml_edit 保注释、广播 `library.updated`）、`reindex`（全量重哈希扫描，异步立即返回）、`rescan`（忽略目录快照强制遍历，仍按 size/mtime 复用哈希）、`refresh_cache`（按范围补派生缓存 + 消失对账）、`cleanup_index`（索引体检：清隐藏文件/ignore 命中/源文件已删的残留位置，只摘索引不动磁盘）、`storage_mode`（元数据存储方案迁移） |
