...
| `src/core/config.rs` | `.hawk/config.toml` 快照（RwLock）与热更 `reload`（返回 `ConfigChange` 差异：ignore/extensions/web 是否变化）；库首次打开生成带注释的默认配置；ignore 匹配器：无 `/` 的模式展开为 `**/p` + `**/p/**`（任意深度同名），分段 glob（`**` 跨目录、段内 `*`/`?`），大小写不敏感；扩展名白名单 `ExtensionFilter`（空 = 不过滤；归一化去点/小写），`is_file_included` = ignore 未命中且扩展名在白名单内（文件入库的唯一判据；目录只用 is_ignored） |
