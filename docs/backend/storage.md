...
`<库标识>` 为库根路径 SHA-256 的前 16 位十六进制，多库互不干扰。缓存目录内布局：

```text
cache/<库标识>/
├── index.db     ← 元数据 SQLite 派生缓存（见下文「元数据缓存」，可删，重启自动重建）
...
- `index.db` 是 TOML 的纯镜像：items / paths / tags / categories 四张表 + 每条记录来源 TOML 的 mtime（`source_mtime`，对账比对依据）。不参与同步，可随时删除，重启后从 TOML 全量重建（仅升级后首次启动慢）
...
- 任何缓存故障（打不开/写失败/读损坏）只退化性能（回到纯 TOML 行为），绝不影响权威数据；schema 版本不符自动重建
