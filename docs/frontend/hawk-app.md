...
thumbSize: number;               // 网格卡片边长偏好（滑杆 120–280）：桌面端会话级、默认 160 不持久化；web 端（浏览器）设备级——记忆到 localStorage（`hawk:thumbSize`，损坏/越界回退），默认最小 120（手机小屏多看几张），变更即写入
